  ...
  return (
    <section
      ...
      className={`relative w-full min-h-screen flex flex-col items-center justify-center ${
        darkMode ? 'bg-gradient-to-b from-[#171c22] to-[#0d1117]' : 'bg-gradient-to-b from-gray-100 to-white'
      } py-20 px-4 sm:px-6`}
    >
      ...
      >
        <motion.h2 
          className={`text-4xl sm:text-5xl font-bold ${darkMode ? 'text-white' : 'text-gray-800'} mb-4`}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      >
        ...
        >
          <a
            ...
            className={`${
              darkMode ? 'text-gray-300 hover:text-[#00BD95]' : 'text-gray-600 hover:text-[#00BD95]'
        ...
        <AnimatePresence>
          {showHistory && (
            ...
            >
              <h3 className={`text-lg font-bold mb-2 ${darkMode ? 'text-white' : 'text-gray-800'}`}>Request History</h3>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-1 rounded text-xs ${
                          item.method === 'GET' ? 'bg-blue-600 text-white' :
                          item.method === 'POST' ? 'bg-green-600 text-white' :
                          item.method === 'PUT' ? 'bg-yellow-600 text-white' :
                          item.method === 'PATCH' ? 'bg-purple-600 text-white' :
                          'bg-red-600 text-white'
                        }`}>
                          {item.method}
                        </span>
                        <span className={`truncate ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{item.url}</span>
                      </div>
                    </motion.div>
                  ))}
                </div>
              ) : (
                <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>No request history yet</p>
              )}
            </motion.div>
          )}
        </AnimatePresence>
              ...
              <div className="max-h-48 overflow-y-auto">
                {examples[apiCategory].map((example, index) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-1 rounded text-xs ${
                        example.method === 'GET' ? 'bg-blue-600 text-white' :
                        example.method === 'POST' ? 'bg-green-600 text-white' :
                        example.method === 'PUT' ? 'bg-yellow-600 text-white' :
                        example.method === 'PATCH' ? 'bg-purple-600 text-white' :
                        'bg-red-600 text-white'
                      }`}>
                        {example.method}
                      </span>
                      <span className={`truncate ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>{example.url}</span>
                    </div>
                    {example.body && (
        ...
        <AnimatePresence>
          {bookmarks.length > 0 && (
            ...
            >
              <h3 className={`text-sm font-bold mb-2 ${darkMode ? 'text-white' : 'text-gray-800'}`}>Bookmarks</h3>
                ...
                {bookmarks.map((bookmark, index) => (
                  ...
                  >
                    <span className={`w-2 h-2 rounded-full ${
                      bookmark.method === 'GET' ? 'bg-blue-500' :
                      bookmark.method === 'POST' ? 'bg-green-500' :
                      bookmark.method === 'PUT' ? 'bg-yellow-500' :
                      bookmark.method === 'PATCH' ? 'bg-purple-500' :
                      'bg-red-500'
                    }`}></span>
                    {bookmark.name}
                  </motion.button>
        ...
        <motion.div variants={itemVariants} className="mb-4 relative">
          <p className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-800'}`}>{activeMethod} Request</p>
          <div className="flex">
            <input
              type="text"
              value={fullUrl}
              onChange={(e) => {
                ...
                const url = new URL(e.target.value.includes("://") ? e.target.value : `http://${e.target.value}`);
                const urlParams = Array.from(new URLSearchParams(url.search).entries()).map(([key, value]) => ({
                  key,
                  value,
                }));
